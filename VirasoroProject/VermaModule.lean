...
lemma VermaModule.apply_hwVec_eq (η : ι → A × 𝕜) (i : ι) :
  ...
  show (η i).1 • (Submodule.Quotient.mk 1 : A ⧸ vermaIdeal η)
      = (algebraMap 𝕜 A (η i).2) • (Submodule.Quotient.mk 1 : A ⧸ vermaIdeal η)
  rw [← Submodule.Quotient.mk_smul, ← Submodule.Quotient.mk_smul, ← sub_eq_zero,
...
@[simp] lemma VermaModule.universalMap_hwVec (η : ι → A × 𝕜)
  ...
  show (vermaIdeal η).liftQ (Ring.smulVectorₗ A hwv) _ (Submodule.Quotient.mk 1) = hwv
