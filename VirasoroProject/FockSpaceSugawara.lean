...
noncomputable def sugawaraRepresentation (α : 𝕜) :
    LieAlgebra.Representation 𝕜 𝕜 (VirasoroAlgebra 𝕜) (HeisenbergVerma 𝕜 α) :=
  sugawaraRepresentation_of_module_uea_heisenbergAlgebra 𝕜 (V := HeisenbergVerma 𝕜 α)
      (fun _ ↦ eventually_jgen_smul_eq_zero ..) (fun _ ↦ HeisenbergVerma.kgen_smul ..)
...
lemma sugawaraRepresentation_lgen_apply
    (α : 𝕜) (n : ℤ) (v : HeisenbergVerma 𝕜 α) :
    HeisenbergVerma.sugawaraRepresentation 𝕜 α (.lgen 𝕜 n) v =
      (2 : 𝕜)⁻¹
...
open HeisenbergAlgebra in
lemma sugawaraRepresentation_lgen_apply_hwVec (α : 𝕜) (n : ℤ) :
    HeisenbergVerma.sugawaraRepresentation 𝕜 α (.lgen 𝕜 n) (hwVec 𝕜 α) =
      (2 : 𝕜)⁻¹
        • ((ιUEA 𝕜 (jgen 𝕜 n) • ιUEA 𝕜 (jgen 𝕜 0) • (hwVec 𝕜 α))
          + (∑ᶠ k < 0, ιUEA 𝕜 (jgen 𝕜 k) • ιUEA 𝕜 (jgen 𝕜 (n-k)) • (hwVec 𝕜 α))) := by
  simp only [sugawaraRepresentation_lgen_apply, ge_iff_le]
  congr 1
  simp only [add_left_inj]
  convert @finsum_eq_single (HeisenbergVerma 𝕜 α) ℤ _ _ 0 ?_
  · simp
  · intro k k_ne_zero
    by_cases k_nn : 0 ≤ k
    · simp only [k_nn, finsum_true]
      rw [jgen_pos_hwVec 𝕜 α (show 0 < k by grind), smul_zero]
    · simp [k_nn]

open HeisenbergAlgebra in
lemma sugawaraRepresentation_lgen_nonneg_apply_hwVec
    (α : 𝕜) {n : ℤ} (n_nn : 0 ≤ n) :
    HeisenbergVerma.sugawaraRepresentation 𝕜 α (.lgen 𝕜 n) (hwVec 𝕜 α) =
      (2 : 𝕜)⁻¹ • ιUEA 𝕜 (jgen 𝕜 n) • ιUEA 𝕜 (jgen 𝕜 0) • (hwVec 𝕜 α) := by
  simp only [sugawaraRepresentation_lgen_apply_hwVec, smul_add]
  convert add_zero ..
  convert smul_zero ..
  · convert finsum_zero with k
    by_cases k_neg : k < 0
    · simp only [k_neg, finsum_true]
      rw [jgen_pos_hwVec 𝕜 α (show 0 < n - k by linarith), smul_zero]
    · simp [k_neg]

/-- The vacuum in the Fock space of charge α has L₀-eigenvalue α²/2. -/
lemma sugawaraRepresentation_lgen_zero_apply_hwVec (α : 𝕜) :
    sugawaraRepresentation 𝕜 α (.lgen 𝕜 0) (hwVec 𝕜 α) =
      (α^2 / 2) • (hwVec 𝕜 α) := by
  rw [sugawaraRepresentation_lgen_nonneg_apply_hwVec 𝕜 α le_rfl]
  simp only [jgen_zero_smul, ← smul_assoc, smul_eq_mul]
  grind

/-- The vacuum in the Fock space of charge α is annihilated by Lₙ for n > 0. -/
lemma sugawaraRepresentation_lgen_pos_apply_hwVec (α : 𝕜)
    {n : ℤ} (n_pos : 0 < n) :
    sugawaraRepresentation 𝕜 α (.lgen 𝕜 n) (hwVec 𝕜 α) = 0 := by
  rw [sugawaraRepresentation_lgen_nonneg_apply_hwVec 𝕜 α n_pos.le]
  convert smul_zero ..
  simp only [jgen_zero_smul]
  rw [smul_comm, jgen_pos_hwVec 𝕜 α n_pos, smul_zero]
