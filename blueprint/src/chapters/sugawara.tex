...
\begin{lemma}[Local finite support for homogeneous normal ordered products]
  ...
  \uses{def:NormalOrdering}
  \lean{VirasoroProject.hasFiniteSupport_smul_pairNO_apply}
  \leanok
  Suppose that $(\Joper_{k})_{k \in \bZ}$ satisfy the local truncation condition
...
\begin{proof}
  ...
  (Lemma~\ref{lem:AlternativeNormalOrdering}) and the commutation relations \tagHeiComm{},
  ...
  Lemma~\ref{lem:CommutatorSugawaraHeisenberg}
...
\begin{lemma}[Local truncation condition for the charged Fock space]
